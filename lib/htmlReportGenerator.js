...
class HtmlReportGenerator {
   ...
   * Uses dcraw to create thumbnails from JPEGs
   */
  async generateSingleThumbnail(imagePath, fileName) {
    ...
    try {
      // For JPEG files, create a symbolic link as thumbnail
      // This avoids duplicating files while maintaining the thumbnail structure
      if (['.jpg', '.jpeg'].includes(ext)) {
        // Create a symlink to the original image
        await fs.ensureDir(path.dirname(thumbnailPath));
        await fs.symlink(imagePath, thumbnailPath).catch(async () => {
          // If symlink fails (Windows), copy the file
          await fs.copy(imagePath, thumbnailPath);
        });
        return thumbnailPath;
      }
    } catch (error) {
    ...
    // Return placeholder if thumbnail creation failed
